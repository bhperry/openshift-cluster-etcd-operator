...
func (EtcdBackupFile) SwaggerDoc() map[string]string {
	return map_EtcdBackupFile
}

var map_EtcdBackupJobReference = map[string]string{
	"name":      "name of the backup job",
	"namespace": "namespace of the backup job",
	"uid":       "uid of the backup job",
}

func (EtcdBackupJobReference) SwaggerDoc() map[string]string {
	return map_EtcdBackupJobReference
}
...
var map_EtcdBackupSpec = map[string]string{
	"nodeName": "nodeName specifies the master node where an etcd backup should be taken. If not specified, a random master node will be selected.",
...
var map_EtcdBackupStatus = map[string]string{
	...
	"job":        "job is a reference to the Job created for the backup.",
	"nodeName":   "nodeName is the master node where the backup snapshot was taken.",
	"files":      "files tracks the path and size of files generated by the etcd backup. Includes both etcd snapshots and static manifests.",
}
...
var map_EtcdBackupStorage = map[string]string{
	"pvc":   "pvc specifies the PersistentVolumeClaim (PVC) which binds a PersistentVolume where the etcd backup file will be saved. The PVC must always be created in the \"openshift-etcd\" namespace. This field is required when the storage type is \"PVC\"",
	"local": "local specifies a host path directory on the master node where the etcd backup file will be saved. This field is required when storage type is \"Local\"",
}

func (EtcdBackupStorage) SwaggerDoc() map[string]string {
	return map_EtcdBackupStorage
}

var map_EtcdBackupStorageLocal = map[string]string{
	"hostPath": "hostPath is a local directory on the master node where the etcd backup file will be saved.",
}

func (EtcdBackupStorageLocal) SwaggerDoc() map[string]string {
	return map_EtcdBackupStorageLocal
}

var map_EtcdBackupStoragePvc = map[string]string{
	"name": "name is a reference to a PVC in the \"openshift-etcd\" namespace where the etcd backup file will be saved.",
	"path": "path is a directory on the volume where the etcd backup file will be saved.",
}

func (EtcdBackupStoragePvc) SwaggerDoc() map[string]string {
	return map_EtcdBackupStoragePvc
}

var map_EtcdBackupPolicy = map[string]string{
	"":       "\n\n# EtcdBackupPolicy sets an automated schedule for taking backups of the etcd cluster\n\nCompatibility level 4: No compatibility is provided, the API can change at any point for any reason. These capabilities should not be used by applications needing long term support.",
	"spec":   "spec holds user settable values for configuration",
	"status": "status holds observed values from the cluster. They may not be overridden.",
}

func (EtcdBackupPolicy) SwaggerDoc() map[string]string {
	return map_EtcdBackupPolicy
}

var map_EtcdBackupPolicyList = map[string]string{
	"": "EtcdBackupPolicyList is a collection of items\n\nCompatibility level 4: No compatibility is provided, the API can change at any point for any reason. These capabilities should not be used by applications needing long term support.",
}

func (EtcdBackupPolicyList) SwaggerDoc() map[string]string {
	return map_EtcdBackupPolicyList
}

var map_EtcdBackupPolicyRetentionRule = map[string]string{
	"type":        "type defined which rule field is set",
	"maxQuantity": "maxQuantity enforces the deletion of backups that exceed the given count.",
	"maxSize":     "maxSize enforces the deletion of backups by the total size of backups on the storage backend. This is a soft threshold. The total size of backups may temporarily exceed the limit when new backups are created.",
}

func (EtcdBackupPolicyRetentionRule) SwaggerDoc() map[string]string {
	return map_EtcdBackupPolicyRetentionRule
}
...
func (EtcdBackupPolicySpec) SwaggerDoc() map[string]string {
	return map_EtcdBackupPolicySpec
}
